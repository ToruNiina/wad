...
TEST_CASE( "std::map<int, std::string> save/load", "[std::map<int, std::string>]" )
{
    const std::map<int, std::string> upto_16{
        {1, "foo"}, {2, "bar"}, {3, "baz"},
        {4, "qux"}, {5, "quux"}
    };
    std::map<int, std::string> upto_u16;
    std::map<int, std::string> upto_u32;
